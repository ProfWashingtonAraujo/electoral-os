...
export class VoterController {
    async stats(_req, res) {
      ...
      `;
            ...
            const topRegions = byRegion
                .filter((r) => r.region)
                .map((r) => ({ region: r.region, count: r._count.region }));
            res.json({
                total,
                statusCounts,
                topRegions,
    ...
    async getAll(req, res) {
        try {
            const { coordinatorId, pollingPlaceId, search, region, neighborhood, supportStatus, status, page, perPage, } = req.query;
            const where = {};
            if (coordinatorId)
                where.coordinatorId = String(coordinatorId);
            if (pollingPlaceId)
                where.pollingPlaceId = String(pollingPlaceId);
            if (region)
                where.region = String(region);
            if (neighborhood)
                where.neighborhood = String(neighborhood);
            if (supportStatus)
                where.supportStatus = String(supportStatus);
            if (status)
    ...
    async create(req, res) {
        try {
            const { name, phone, whatsapp, address, neighborhood, city, region, voterRegistration, electoralZone, electoralSection, supportStatus, notes, registrationSource, status, pollingPlaceId, coordinatorId } = req.body;
